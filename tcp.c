...
tcp_segment_arrives(struct tcp_segment_info *seg, uint8_t flags, uint8_t *data, size_t len, struct ip_endpoint *local, struct ip_endpoint *foreign)
{
    ...
    case TCP_PCB_STATE_SYN_SENT:
         ...
         */
        if (TCP_FLG_ISSET(flags, TCP_FLG_ACK)) {
            /* 送信していないシーケンス番号に対する ACK だったら RST を返す */
            if (seg->ack <= pcb->iss || seg->ack > pcb->snd.nxt) {
                tcp_output_segment(seg->ack, 0, TCP_FLG_RST, 0, NULL, 0, local, foreign);
                return;
            }
            /* まだ ACK の応答が得られていないシーケンス番号に対するものだったら受け入れる */
        ...
        if (TCP_FLG_ISSET(flags, TCP_FLG_SYN)) {
            ...
            if (acceptable) {
                /* ACK を受け入れた際の処理 */
                /* - 未確認のシーケンス番号を更新（ACK の値は「次に受信すべきシーケンス番号」を示すので ACK の値と同一のシーケンス番号の確認はとれていない） */
                /* - 再送キューから ACK によって到達が確認できている TCP セグメントを除去 */
                pcb->snd.una = seg->ack;
            ...
            if (pcb->snd.una > pcb->iss) {
                ...
                /* 同時オープン (両方が同時に SYN を送った場合) に対処するためのコード */
                pcb->state = TCP_PCB_STATE_SYN_RECEIVED;
                tcp_output(pcb, TCP_FLG_SYN | TCP_FLG_ACK, NULL, 0);
    ...
    case TCP_PCB_STATE_CLOSE_WAIT:
        ...
        switch (pcb->state) {
        case TCP_PCB_STATE_FIN_WAIT1:
            /* FIN への ACK を受信したら FIN_WAIT2 へ遷移して相手からの送信を待つ */
        ...
        break;
    case TCP_PCB_STATE_LAST_ACK:
        /* FIN への ACK を受信したらコントロールブロックを削除して CLOSED へ遷移 */
        if (seg->ack == pcb->snd.nxt) {
            pcb->state = TCP_PCB_STATE_CLOSED;
            tcp_pcb_release(pcb);
        }
    ...
    if (TCP_FLG_ISSET(flags, TCP_FLG_FIN)) {
        ...
        pcb->rcv.nxt = seg->seq + 1;
        /* Passive Close Start: ESTABLISHED 状態で FIN を受信したらACK を送信 */
        tcp_output(pcb, TCP_FLG_ACK, NULL, 0);
        switch (pcb->state) {
        case TCP_PCB_STATE_SYN_RECEIVED:
        case TCP_PCB_STATE_ESTABLISHED:
...
tcp_open_rfc793(struct ip_endpoint *local, struct ip_endpoint *foreign, int active)
{
    ...
    if (active) {
        ...
        /* まだ ACK の確認が得られていないシーケンス番号として設定 */
        pcb->snd.una = pcb->iss;
        /* 次に送信すべきシーケンス番号を設定 */
        pcb->snd.nxt = pcb->iss + 1;
        /* SYN-SENT 状態へ移行 */
...
tcp_close(int id)
{
    ...
    /* コネクション切断 */
    switch (pcb->state) {
    /* Active Close: ESTABLISHED 状態で CLOSE コマンドが呼ばれたら FIN を送信して FIN_WAIT1 へ遷移 */
    case TCP_PCB_STATE_ESTABLISHED:
        tcp_output(pcb, TCP_FLG_ACK | TCP_FLG_FIN,  NULL, 0);
        pcb->snd.nxt++;
        pcb->state = TCP_PCB_STATE_FIN_WAIT1;
        break;
    /* Passive Close: Close コマンドが呼ばれたら FIN を送信して LAST_ACK へ遷移 */
    case TCP_PCB_STATE_CLOSE_WAIT:
        tcp_output(pcb, TCP_FLG_ACK | TCP_FLG_FIN, NULL, 0);
        pcb->snd.nxt++;
